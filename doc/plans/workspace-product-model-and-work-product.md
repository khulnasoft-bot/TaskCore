...
Taskcore needs to support two very different but equally valid ways of working:
...
Today, Taskcore already has the beginnings of this model:
...
- making Taskcore feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing Taskcore users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted Taskcore deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning Taskcore into a full code review product
...
This object must be recorded explicitly so that Taskcore can:
...
Taskcore should treat PRs as a type of work product linked back to:
...
Taskcore already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted Taskcore must be a first-class deployment mode

Taskcore cannot assume that it is running on the same machine as the code.

In cloud deployments, Taskcore may:
...
- `Runtime service`: a process or service Taskcore owns or tracks for a workspace
...
It also lets cloud-hosted Taskcore deployments point at codebases and remotes without pretending the Taskcore host has direct filesystem access.
...
Without an explicit `execution workspace` record, Taskcore has nowhere stable to attach:
...
Taskcore needs a single place to show:
...
- Taskcore-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without Taskcore owning the execution host
...
## 1. Taskcore host and execution host must be decoupled
...
without the Taskcore host itself running local git or local preview processes.
...
Taskcore should not copy its product framing wholesale, but there are several good patterns worth applying.
...
This is a strong pattern for Taskcore.
...
Taskcore should keep workspace automation centered on a small set of hooks:
...
Taskcore should provide the equivalent visibility in the project `Code` surface:
...
Taskcore should treat execution workspaces as the natural home for:
...
Taskcore should ensure that execution workspaces and work product have clean structured API surfaces, not just UI-only representation.
...
Taskcore should continue treating cleanup policy as part of the core workspace model:
...
Taskcore should not adopt these Worktrunk assumptions as universal product rules:

- every execution workspace is a local git worktree
- the Taskcore host has direct shell and filesystem access
- every workflow is merge-centric
- every user wants developer-tool-level workspace detail in the main navigation

### Product implication

Taskcore should borrow Worktrunk's good execution patterns while keeping the broader Taskcore model:
...
That keeps Taskcore focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted Taskcore deployments remain first-class

That is a better fit for Taskcore than either extreme:
