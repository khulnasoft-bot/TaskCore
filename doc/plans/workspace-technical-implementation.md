...
This document translates [workspace-product-model-and-work-product.md](/Users/dotta/taskcore-subissues/doc/plans/workspace-product-model-and-work-product.md) into an implementation-ready engineering plan.
...
This repo already uses `TASKCORE_DEPLOYMENT_MODE` for auth/deployment behavior (`local_trusted | authenticated`).
...
- `TASKCORE_EXECUTION_TOPOLOGY=local|cloud|hybrid`
...
- `local`
  - Taskcore may create host-local worktrees, processes, and paths
- `cloud`
  - Taskcore should assume no durable host-local execution workspace management
...
- `provider text not null`
  - `taskcore | github | vercel | s3 | custom`
...
- use `TASKCORE_EXECUTION_TOPOLOGY` as a defaulting guardrail
