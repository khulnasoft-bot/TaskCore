...
Taskcore already treats budgets as a core control-plane responsibility:
...
This plan defines the concrete budgeting model Taskcore should implement next.

## Product Goals

Taskcore should let operators:
...
For the next implementation pass, Taskcore should enforce these defaults:
...
Taskcore should separate subscription-included usage from billed spend:
...
Taskcore should have two threshold classes:
...
Taskcore must know that a project is budget-paused and enforce it.
...
Taskcore must also block execution before new work starts.
