# Agent OS Technical Report for Taskcore
...
`agent-os` is not a competitor to Taskcore's core product. It is an execution substrate: an embedded, VM-like runtime for agents, tools, filesystems, and session orchestration. Taskcore is a control plane: company scoping, task hierarchy, approvals, budgets, activity logs, workspaces, and governance.

The strongest takeaway is not "copy agent-os wholesale." The strongest takeaway is that Taskcore could selectively use its runtime ideas to improve local agent execution safety, reproducibility, and portability while keeping all company/task/governance logic in Taskcore.

My recommendation is:

1. Do not merge agent-os concepts into the Taskcore core product model.
...
4. Do not import its workflow/cron/queue abstractions into Taskcore core until they are reconciled with Taskcore's issue/comment/governance model.
...
That gives `agent-os` a much stronger isolation story than Taskcore's current "launch a host CLI in a workspace" local adapter approach.
...
## What Taskcore can learn from it

### 1. A safer local execution substrate

Taskcore's local adapters currently run host CLIs in managed workspaces and rely on adapter-specific behavior plus process-level controls. That is pragmatic, but weakly isolated.
...
This fits Taskcore because it improves execution safety without changing the control-plane model.

### 2. Snapshotted execution roots instead of only mutable workspaces

Taskcore already has strong execution-workspace concepts, but they are repo/worktree-centric. `agent-os` adds a stronger "start from known lower layers, write into a disposable upper layer" model.
...
Taskcore has governance at the company/task level:
...
It has less structure at the runtime capability level. `agent-os` offers a clear vocabulary that Taskcore could adopt even without adopting the runtime itself:
...
Taskcore's plugin system and adapters already have the beginnings of a controlled extension surface. `agent-os` reinforces the value of exposing capabilities as typed tools rather than raw shell access.

Concrete Taskcore uses:
...
This aligns well with Taskcore's governance story.

### 5. Better adapter normalization around sessions and capabilities

Taskcore's adapter contract already supports execution results, session params, environment tests, skill syncing, quota windows, and optional `detectModel()`. But much of the per-agent behavior is still adapter-specific.
...
Taskcore does not need ACP everywhere, but it would benefit from a more formal internal session capability model inspired by this.
...
Taskcore can adopt that philosophy directly:
...
## What does not fit Taskcore well

### 1. Its built-in orchestration primitives overlap the wrong layer

`agent-os` includes cron/session/workflow style primitives inside the runtime package. Taskcore already has higher-level orchestration concepts:
...
If Taskcore copied `agent-os` cron/workflow/queue ideas directly into core, we would likely duplicate orchestration across two layers. That would blur ownership and make debugging harder.

Taskcore should keep orchestration authoritative at the control-plane layer.
...
Those are Taskcore's differentiators. They should not be displaced by runtime abstractions.
...
The permission model is good, but it is low-level. Taskcore would still need to answer:
...
- real integration work for Codex/Cursor/Gemini-style Taskcore adapters

So the main near-term value is not universal replacement. It is selective use where compatibility is strong.

## Concrete recommendations for Taskcore
...
- keep Taskcore heartbeat/task/workspace/budget logic unchanged
...
- Taskcore's Codex adapter is already important and carries repo-specific behavior
...
- Taskcore still records logs, summaries, cost metadata, and issue comments normally
...
Even without using `agent-os`, Taskcore should consider standardizing adapter/runtime permissions around a vocabulary like:
...
Taskcore should evaluate whether some execution workspaces can be backed by:
...
Taskcore plugins and adapters should continue moving toward explicit typed tools over ad hoc shell access. `agent-os` confirms that this is the right direction.
...
### Recommendation E: do not import runtime-level workflows into Taskcore core

Taskcore should not copy `agent-os` cron/workflow/queue concepts into core orchestration yet.
...
- moving Taskcore orchestration into agent-os workflows
...
`agent-os` is useful to Taskcore as an execution technology reference, not as a product model.

Taskcore should treat it the same way it treats sandboxes or agent CLIs:
