...
Turn the `agent-os` research into a low-risk Taskcore execution plan that preserves Taskcore's control-plane model while testing the few runtime ideas that appear worth adopting.

## Decision summary

Taskcore should not absorb `agent-os` as a product model or orchestration layer.

Taskcore should evaluate `agent-os` in three narrow areas:
...
This work is successful when Taskcore has:
...
- a proposed runtime capability model that fits current Taskcore adapters
...
- replace Taskcore heartbeats, issues, comments, approvals, or budgets with `agent-os` primitives
- introduce Rust/sidecar requirements for all local execution paths
- migrate all adapters at once
- add runtime workflow/queue abstractions to Taskcore core

## Existing Taskcore integration points
...
- prove that Taskcore can drive one local agent through an `agent-os` runtime without breaking heartbeat semantics
...
- keep Taskcore's existing heartbeat, issue, workspace, and comment flow authoritative
...
- introduce a Taskcore-native capability vocabulary without coupling the product to `agent-os`
...
- determine whether a layered/snapshotted root model improves some Taskcore workloads
...
- identify where Taskcore should prefer explicit typed tools over ambient shell access
...
- one concrete proposal for typed-tool adoption in Taskcore
...
- runtime experimentation could blur the boundary between Taskcore as control plane and Taskcore as execution platform
