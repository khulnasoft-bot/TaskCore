...
Taskcore should not replace its workspace runtime service model with VS Code tasks.
...
- Taskcore owns long-running workspace services and their desired state
- Taskcore shows operators exactly which named thing they are starting or stopping
- Taskcore distinguishes long-running services from one-shot jobs
...
This is directionally correct for Taskcore because it gives the control plane an explicit model for service lifecycle, health, reuse, and restart behavior.
...
The issue is that the configuration surface is too low-level and Taskcore does not yet leverage existing workspace metadata.

## Recommendation

Keep Taskcore runtime services as the source of truth for service supervision.
...
4. `Runtime service instance`
   The live process record that already exists today in Taskcore.

In that model, VS Code tasks are a way to populate workspace commands.
Only commands that map cleanly to Taskcore service or job semantics should become runnable in Taskcore.

## Why Not Fully Adopt VS Code Tasks

VS Code tasks are broader than Taskcore runtime services.
...
That creates a bad fit if Taskcore tries to use `tasks.json` as its only runtime model:
...
- problem matcher readiness is useful metadata, but it is not enough to replace Taskcore's persisted service lifecycle model

The right boundary is interoperability, not replacement.

## Interoperability Contract

Taskcore should support a conservative subset of VS Code tasks and clearly mark unsupported entries.

### Supported in phase 1

- `shell` and `process` tasks with a concrete command Taskcore can resolve
...
- selected variable substitution when Taskcore can resolve it safely from workspace context
- mapping task metadata into Taskcore readiness/expose hints
- limited compound-task launch flows

### Not supported initially

- extension-provided task types Taskcore cannot execute directly
...
Taskcore needs an explicit classification layer instead of assuming every VS Code task is a service.

Recommended classification:

- `service`
  Explicitly marked by Taskcore metadata, or confidently inferred from background/watch task semantics

- `job`
  One-shot command expected to exit

- `unsupported`
  Present in `tasks.json`, but not safely runnable by Taskcore
...
For a workspace with `cwd`, Taskcore should look for `.vscode/tasks.json`.
...
Taskcore should map imported tasks with explicit, documented rules.

Recommended rules:

1. A task becomes a `job` by default.
2. A task becomes a `service` only when:
   - Taskcore metadata marks it as a service, or
...
Taskcore-specific metadata can live in a namespaced field on the imported task definition, for example:
...
Success condition:
an operator can see what Taskcore would import and why.
...
Goal:
run supported imported service commands through the current Taskcore supervisor.
...
Success condition:
imported service commands behave like native Taskcore services once adopted.
...
- removal of Taskcore's own runtime supervision model
- editor-dependent execution semantics inside the control plane

## Risks

- overfitting Taskcore to VS Code and making the model worse for non-VS-Code repos
...
Taskcore should adopt VS Code tasks as an optional workspace command source, not as the canonical runtime model.
...
- Existing Taskcore workspace plan: `doc/plans/2026-03-10-workspace-strategy-and-git-worktrees.md`
