...
- New config/env settings:
  ...
  - `TASKCORE_AGENT_JWT_TTL_SECONDS` (default: `172800`)
  - `TASKCORE_AGENT_JWT_ISSUER` (default: `taskcore`)
  - `TASKCORE_AGENT_JWT_AUDIENCE` (default: `taskcore-api`)
...
1. In:
   ...
   inject `TASKCORE_API_KEY` from context token.

- Preserve existing behavior for explicit user-defined env vars in `adapterConfig.env`:
  - if user already sets `TASKCORE_API_KEY`, do not overwrite it.
- Continue injecting:
  - `TASKCORE_AGENT_ID`
  - `TASKCORE_COMPANY_ID`
  - `TASKCORE_API_URL`
...
- Local adapters authenticate without manual `TASKCORE_API_KEY` config.
