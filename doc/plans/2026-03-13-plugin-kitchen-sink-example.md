...
Add a new first-party example plugin, `Kitchen Sink (Example)`, that demonstrates every currently implemented Taskcore plugin API surface in one place.
...
The plugin is successful if a contributor can install it and, without reading the SDK first, discover and exercise the current plugin runtime surface area from inside Taskcore.
...
- package: `@taskcore/plugin-kitchen-sink-example`
- plugin id: `taskcore.kitchen-sink-example` or `taskcore-kitchen-sink-example`

Recommendation: use `taskcore-kitchen-sink-example` to match current in-repo example naming style.
...
- subscribe to a core Taskcore event such as `issue.created`
...
Recommendation: default to a Taskcore-local endpoint or a stable public echo endpoint to avoid flaky docs.
