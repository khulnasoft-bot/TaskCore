...
For `claude_local` and `codex_local`, Taskcore assumes the CLI is already installed and authenticated on the host machine.
...
Taskcore stores resumable session state per `(agent, taskKey, adapterType)`.
