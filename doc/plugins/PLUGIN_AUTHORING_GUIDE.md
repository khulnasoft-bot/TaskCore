...
This guide describes the current, implemented way to create a Taskcore plugin in this repo.
...
- Plugin UI runs as same-origin JavaScript inside the main Taskcore app.
...
```bash
pnpm --filter @taskcore/create-taskcore-plugin build
node packages/plugins/create-taskcore-plugin/dist/index.js @yourscope/plugin-name --output ./packages/plugins/examples
```

For a plugin that lives outside the Taskcore repo:

```bash
pnpm --filter @taskcore/create-taskcore-plugin build
node packages/plugins/create-taskcore-plugin/dist/index.js @yourscope/plugin-name \
...
Inside this monorepo, the scaffold uses `workspace:*` for `@taskcore/plugin-sdk`.

Outside this monorepo, the scaffold snapshots `@taskcore/plugin-sdk` from the local Taskcore checkout into a `.taskcore-sdk/` tarball so you can build and test a plugin without publishing anything to npm first.
...
For local development, install it into Taskcore from an absolute local path through the plugin manager or API. The server supports local filesystem installs and watches local-path plugins for file changes so worker restarts happen automatically after rebuilds.
...
- `usePluginToast`
- `useHostContext`
- typed slot props from `@taskcore/plugin-sdk/ui`
