...
This document explains how Taskcore interprets issue assignment, issue status, execution runs, wakeups, parent/sub-issue structure, and blocker relationships.

`doc/SPEC-implementation.md` remains the V1 contract. This document is the detailed execution model behind that contract.

## 1. Core Model

Taskcore separates four concepts that are easy to blur together:
...
This is a hard invariant. Taskcore is single-assignee by design.

## 3. Status Semantics

Taskcore issue statuses are not just UI labels. They imply different expectations about ownership and execution.
...
- for agent-assigned work, Taskcore may still need a wake path to ensure the assignee actually sees it
...
- Taskcore can wake the assignee
- Taskcore can track runs linked to the issue
- Taskcore can recover some lost execution state after crashes/restarts

### User-owned issues

User-owned issues are not executed by the heartbeat scheduler.

- Taskcore can track the ownership and status
- Taskcore cannot rely on heartbeat/run semantics to keep them moving
...
Taskcore already clears stale execution locks and can adopt some stale checkout locks when the original run is gone.

## 6. Parent/Sub-Issue vs Blockers

Taskcore uses two different relationships for different jobs.
...
Blocked issues should stay idle while blockers remain unresolved. Taskcore should not create a queued heartbeat run for that issue until the final blocker is done and the `issue_blockers_resolved` wake can start real work.

If a parent is truly waiting on a child, model that with blockers. Do not rely on the parent/child relationship alone.

## 7. Consistent Execution Path Rules

For agent-assigned, non-terminal, actionable issues, Taskcore should not leave work in a state where nobody is working it and nothing will wake it.
...
Taskcore now treats crash/restart recovery as a stranded-assigned-work problem, not just a stranded-run problem.
...
- if the latest issue-linked run failed/timed out/cancelled and no live execution path remains, Taskcore queues one automatic assignment recovery wake
- if that recovery wake also finishes and the issue is still stranded, Taskcore moves the issue to `blocked` and posts a visible comment
...
- Taskcore queues one automatic continuation wake
- if that continuation wake also finishes and the issue is still stranded, Taskcore moves the issue to `blocked` and posts a visible comment
...
On startup and on the periodic recovery loop, Taskcore now does three things in sequence:
...
That is the execution contract Taskcore should present to operators.
