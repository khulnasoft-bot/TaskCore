...
description: >
  Create new agents in Taskcore with governance-aware hiring. Use when you need
...
```sh
curl -sS "$TASKCORE_API_URL/api/agents/me" \
  -H "Authorization: Bearer $TASKCORE_API_KEY"
```

2. Discover available adapter configuration docs for this Taskcore instance.

```sh
curl -sS "$TASKCORE_API_URL/llms/agent-configuration.txt" \
  -H "Authorization: Bearer $TASKCORE_API_KEY"
```
...
```sh
curl -sS "$TASKCORE_API_URL/llms/agent-configuration/claude_local.txt" \
  -H "Authorization: Bearer $TASKCORE_API_KEY"
```
...
```sh
curl -sS "$TASKCORE_API_URL/api/companies/$TASKCORE_COMPANY_ID/agent-configurations" \
  -H "Authorization: Bearer $TASKCORE_API_KEY"
```
...
```sh
curl -sS "$TASKCORE_API_URL/api/approvals/<approval-id>" \
  -H "Authorization: Bearer $TASKCORE_API_KEY"

curl -sS -X POST "$TASKCORE_API_URL/api/approvals/<approval-id>/comments" \
  -H "Authorization: Bearer $TASKCORE_API_KEY" \
  -H "Content-Type: application/json" \
...
```sh
curl -sS -X POST "$TASKCORE_API_URL/api/issues/<issue-id>/approvals" \
  -H "Authorization: Bearer $TASKCORE_API_KEY" \
  -H "Content-Type: application/json" \
...
```sh
curl -sS "$TASKCORE_API_URL/api/approvals/$TASKCORE_APPROVAL_ID" \
  -H "Authorization: Bearer $TASKCORE_API_KEY"

curl -sS "$TASKCORE_API_URL/api/approvals/$TASKCORE_APPROVAL_ID/issues" \
  -H "Authorization: Bearer $TASKCORE_API_KEY"
```
