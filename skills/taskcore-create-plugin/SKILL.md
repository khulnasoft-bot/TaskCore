---
name: taskcore-create-plugin
description: >
  Create new Taskcore plugins with the current alpha SDK/runtime. Use when
...
# Create a Taskcore Plugin

Use this skill when the task is to create, scaffold, or document a Taskcore plugin.
...
node packages/plugins/create-taskcore-plugin/dist/index.js <npm-package-name> --output <target-dir>
```

For a plugin that lives outside the Taskcore repo, pass `--sdk-path` and let the scaffold snapshot the local SDK/shared packages into `.taskcore-sdk/`:

```bash
pnpm --filter @taskcore/create-taskcore-plugin build
node packages/plugins/create-taskcore-plugin/dist/index.js @acme/plugin-name \
...
- is installed into Taskcore from an absolute local path during development
