 ...
 *
 * Plugin bundles are built with `external: ["@taskcore/plugin-sdk/ui", "react", "react-dom"]`,
 ...
 *    host's global bridge registry (`globalThis.__taskcorePluginBridge__`)
...
function getShimBlobUrl(specifier: "react" | "react-dom" | "react-dom/client" | "react/jsx-runtime" | "sdk-ui"): string {
  ...
  switch (specifier) {
    case "react":
      source = `
        const R = globalThis.__taskcorePluginBridge__?.react;
        export default R;
    ...
    case "react-dom/client":
      source = `
        const RD = globalThis.__taskcorePluginBridge__?.reactDom;
        export default RD;
    ...
    case "sdk-ui":
      source = `
        const SDK = globalThis.__taskcorePluginBridge__?.sdkUi ?? {};
 ...
 * - `import { ... } from "@taskcore/plugin-sdk/ui";`
 *
...
function rewriteBareSpecifiers(source: string): string {
  ...
  const rewrites: Record<string, string> = {
    '"@taskcore/plugin-sdk/ui"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@taskcore/plugin-sdk/ui'": `'${getShimBlobUrl("sdk-ui")}'`,
    '"@taskcore/plugin-sdk/ui/hooks"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@taskcore/plugin-sdk/ui/hooks'": `'${getShimBlobUrl("sdk-ui")}'`,
...
async function importPluginModule(url: string): Promise<Record<string, unknown>> {
  ...
  if (!globalThis.__taskcorePluginBridge__) {
 ...
 * self-register via `window.taskcorePlugins.registerReactComponent()`.
 ...
 * of `@taskcore/plugin-sdk/ui`, `react`, and `react-dom` resolve to the
