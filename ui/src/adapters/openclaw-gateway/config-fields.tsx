...
export function OpenClawGatewayConfigFields({
  ...
  return (
    <>
      ...
      {!isCreate && (
        <>
          <Field label="Taskcore API URL override">
            <DraftInput
              value={
                eff(
                  "adapterConfig",
                  "taskcoreApiUrl",
                  String(config.taskcoreApiUrl ?? ""),
                )
              }
              onCommit={(v) => mark("adapterConfig", "taskcoreApiUrl", v || undefined)}
              immediate
              className={inputClass}
              placeholder="https://taskcore.example"
            />
          </Field>
          ...
          <Field label="Claimed API key path">
            <DraftInput
              ...
              immediate
              className={inputClass}
              placeholder="~/.openclaw/workspace/taskcore-claimed-api-key.json"
            />
          </Field>
          ...
          {sessionStrategy === "fixed" && (
            <Field label="Session key">
              <DraftInput
                value={eff("adapterConfig", "sessionKey", String(config.sessionKey ?? "taskcore"))}
          ...
          <Field label="Device auth">
            <div className="text-xs text-muted-foreground leading-relaxed">
              Always enabled for gateway agents. Taskcore persists a device key during onboarding so pairing approvals
