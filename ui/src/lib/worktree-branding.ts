...
export function getWorktreeUiBranding(): WorktreeUiBranding | null {
  if (readMetaContent("taskcore-worktree-enabled") !== "true") return null;

  const name = readMetaContent("taskcore-worktree-name");
  const color = normalizeHexColor(readMetaContent("taskcore-worktree-color"));
  ...
  return {
    ...
    name,
    color,
    textColor: normalizeHexColor(readMetaContent("taskcore-worktree-text-color")) ?? pickReadableTextColor(color),
  };
}
