...
const hermesLocalAdapter: ServerAdapterModule = {
  ...
  execute: async (ctx) => {
    ...
    const explicitApiKey =
      typeof existingEnv.TASKCORE_API_KEY === "string" && existingEnv.TASKCORE_API_KEY.trim().length > 0;
    ...
    const authGuardPrompt = [
      "Taskcore API safety rule:",
      "Use Authorization: Bearer $TASKCORE_API_KEY on every Taskcore API request.",
      "Use X-Taskcore-Run-Id: $TASKCORE_RUN_ID on every Taskcore API request that writes or mutates data, including comments and issue updates.",
      "Never use a board, browser, or local-board session for Taskcore API writes.",
    ].join("\n");
    ...
    const patchedConfig: Record<string, unknown> = {
      ...
      env: {
        ...existingEnv,
        ...(!explicitApiKey ? { TASKCORE_API_KEY: normalizedCtx.authToken } : {}),
        TASKCORE_RUN_ID: normalizedCtx.runId,
      },
    };
