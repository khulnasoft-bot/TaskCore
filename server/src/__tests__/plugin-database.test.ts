...
describeEmbeddedPostgres("plugin database namespaces", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("taskcore-plugin-db-");
    db = createDb(tempDb.connectionString);
  }, 20_000);

  afterEach(async () => {
    for (const pluginKey of ["taskcore.dbtest", "taskcore.escape"]) {
  ...
  async function createPluginPackage(manifest: TaskcorePluginManifestV1, migrationSql: string) {
    const packageRoot = await mkdtemp(path.join(os.tmpdir(), "taskcore-plugin-package-"));
  ...
  async function installPluginRecord(manifest: TaskcorePluginManifestV1) {
  ...
  function manifest(pluginKey = "taskcore.dbtest"): TaskcorePluginManifestV1 {
    return {
      id: pluginKey,
      apiVersion: 1,
      version: "1.0.0",
  ...
  it("records a failed migration when SQL escapes the plugin namespace", async () => {
    const pluginManifest = manifest("taskcore.escape");
    const packageRoot = await createPluginPackage(
      pluginManifest,
