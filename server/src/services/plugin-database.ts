...
export function pluginDatabaseService(db: Db) {
  ...
  async function ensureNamespace(pluginId: string, manifest: TaskcorePluginManifestV1) {
  ...
  return {
    ...
    async applyMigrations(pluginId: string, manifest: TaskcorePluginManifestV1, packageRoot: string) {
      if (!manifest.database) return null;
