...
export function heartbeatService(db: Db) {
    ...
    for (const candidate of candidates) {
      ...
      await issuesSvc.addComment(
        ...
        [
          ...
          "",
          `Taskcore found this issue is blocking ${blockingLinks} but had no assignee, so no heartbeat could pick it up.`,
          "",
  ...
  function buildLivenessEscalationDescription(finding: IssueLivenessFinding) {
    return [
      "Taskcore detected a harness-level issue graph liveness incident.",
      "",
  ...
  function buildLivenessOriginalIssueComment(finding: IssueLivenessFinding, escalation: typeof issues.$inferSelect) {
    return [
      "Taskcore detected a harness-level liveness incident in this issue's dependency graph.",
      "",
  ...
  async function executeRun(runId: string) {
    ...
    if (continuationSummary) {
      ...
      delete context.taskcoreContinuationSummary;
    }
  ...
  function buildImmediateExecutionPathRecoveryComment(input: {
    ...
    if (input.status === "todo") {
      return (
        "Taskcore automatically retried dispatch for this assigned `todo` issue during terminal run recovery, " +
        `but it still has no live execution path.${failureSummary ?? ""} ` +
        "Moving it to `blocked` so it is visible for intervention."
      );
    }
    ...
    return (
      "Taskcore automatically retried continuation for this assigned `in_progress` issue during terminal run " +
