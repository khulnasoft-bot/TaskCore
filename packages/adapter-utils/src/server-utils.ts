...
export const DEFAULT_TASKCORE_AGENT_PROMPT_TEMPLATE = [
...
export interface TaskcoreSkillEntry {
  key: string;
  runtimeName: string;
  source: string;
  required?: boolean;
...
interface PersistentSkillSnapshotOptions {
  adapterType: string;
  availableEntries: TaskcoreSkillEntry[];
...
> {
  if (entry.required) {
    return {
      origin: "taskcore_required",
      originLabel: "Required by Taskcore",
      readOnly: false,
    };
  }
  return {
    origin: "company_managed",
    originLabel: "Managed by Taskcore",
    readOnly: false,
  };
}
...
type TaskcoreWakeExecutionPrincipal = {
  type: "agent" | "user" | null;
  agentId: string | null;
  userId: string | null;
};

type TaskcoreWakeExecutionStage = {
  wakeRole: "reviewer" | "approver" | "executor" | null;
  stageId: string | null;
  stageType: string | null;
  currentParticipant: TaskcoreWakeExecutionPrincipal | null;
  returnAssignee: TaskcoreWakeExecutionPrincipal | null;
  lastDecisionOutcome: string | null;
  allowedActions: string[];
};

type TaskcoreWakeComment = {
  id: string | null;
  issueId: string | null;
  body: string;
...
type TaskcoreWakeContinuationSummary = {
  key: string | null;
  title: string | null;
  body: string;
  bodyTruncated: boolean;
...
type TaskcoreWakeLivenessContinuation = {
  attempt: number | null;
...
type TaskcoreWakeChildIssueSummary = {
  id: string | null;
  identifier: string | null;
  title: string | null;
  status: string | null;
...
type TaskcoreWakeBlockerSummary = {
  id: string | null;
  identifier: string | null;
  title: string | null;
  status: string | null;
...
type TaskcoreWakePayload = {
  reason: string | null;
  issue: TaskcoreWakeIssue | null;
  checkedOutByHarness: boolean;
  dependencyBlockedInteraction: boolean;
  unresolvedBlockerIssueIds: string[];
  unresolvedBlockerSummaries: TaskcoreWakeBlockerSummary[];
  executionStage: TaskcoreWakeExecutionStage | null;
  continuationSummary: TaskcoreWakeContinuationSummary | null;
  livenessContinuation: TaskcoreWakeLivenessContinuation | null;
  childIssueSummaries: TaskcoreWakeChildIssueSummary[];
  childIssueSummaryTruncated: boolean;
  commentIds: string[];
  latestCommentId: string | null;
  comments: TaskcoreWakeComment[];
...
function normalizeTaskcoreWakeComment(value: unknown): TaskcoreWakeComment | null {
...
function normalizeTaskcoreWakeContinuationSummary(value: unknown): TaskcoreWakeContinuationSummary | null {
...
function normalizeTaskcoreWakeLivenessContinuation(value: unknown): TaskcoreWakeLivenessContinuation | null {
...
function normalizeTaskcoreWakeChildIssueSummary(value: unknown): TaskcoreWakeChildIssueSummary | null {
...
function normalizeTaskcoreWakeBlockerSummary(value: unknown): TaskcoreWakeBlockerSummary | null {
...
function normalizeTaskcoreWakeExecutionPrincipal(value: unknown): TaskcoreWakeExecutionPrincipal | null {
...
function normalizeTaskcoreWakeExecutionStage(value: unknown): TaskcoreWakeExecutionStage | null {
  ...
  const allowedActions = Array.isArray(stage.allowedActions)
    ...
    : [];
  const currentParticipant = normalizeTaskcoreWakeExecutionPrincipal(stage.currentParticipant);
  const returnAssignee = normalizeTaskcoreWakeExecutionPrincipal(stage.returnAssignee);
...
export function normalizeTaskcoreWakePayload(value: unknown): TaskcoreWakePayload | null {
  const payload = parseObject(value);
  const comments = Array.isArray(payload.comments)
    ? payload.comments
        .map((entry) => normalizeTaskcoreWakeComment(entry))
        .filter((entry): entry is TaskcoreWakeComment => Boolean(entry))
    : [];
  ...
  const commentIds = Array.isArray(payload.commentIds)
    ? payload.commentIds
        ...
        .map((entry) => entry.trim())
    : [];
  const executionStage = normalizeTaskcoreWakeExecutionStage(payload.executionStage);
  const continuationSummary = normalizeTaskcoreWakeContinuationSummary(payload.continuationSummary);
  const livenessContinuation = normalizeTaskcoreWakeLivenessContinuation(payload.livenessContinuation);
  const childIssueSummaries = Array.isArray(payload.childIssueSummaries)
    ? payload.childIssueSummaries
        .map((entry) => normalizeTaskcoreWakeChildIssueSummary(entry))
        .filter((entry): entry is TaskcoreWakeChildIssueSummary => Boolean(entry))
    : [];
  ...
  const unresolvedBlockerSummaries = Array.isArray(payload.unresolvedBlockerSummaries)
    ? payload.unresolvedBlockerSummaries
        .map((entry) => normalizeTaskcoreWakeBlockerSummary(entry))
        .filter((entry): entry is TaskcoreWakeBlockerSummary => Boolean(entry))
    : [];

  if (comments.length === 0 && commentIds.length === 0 && childIssueSummaries.length === 0 && unresolvedBlockerIssueIds.length === 0 && unresolvedBlockerSummaries.length === 0 && !executionStage && !continuationSummary && !livenessContinuation && !normalizeTaskcoreWakeIssue(payload.issue)) {
    return null;
  }

  return {
    reason: asString(payload.reason, "").trim() || null,
    issue: normalizeTaskcoreWakeIssue(payload.issue),
...
export function stringifyTaskcoreWakePayload(value: unknown): string | null {
  const normalized = normalizeTaskcoreWakePayload(value);
  if (!normalized) return null;
  return JSON.stringify(normalized);
}

export function renderTaskcoreWakePrompt(
  value: unknown,
...
export function buildTaskcoreEnv(agent: { id: string; companyId: string }): Record<string, string> {
  ...
  const apiUrl = process.env.TASKCORE_API_URL ?? `http://${runtimeHost}:${runtimePort}`;
  vars.TASKCORE_API_URL = apiUrl;
  return vars;
}
...
export async function resolveTaskcoreSkillsDir(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<string | null> {
  const candidates = [
    ...TASKCORE_SKILL_ROOT_RELATIVE_CANDIDATES.map((relativePath) => path.resolve(moduleDir, relativePath)),
...
export async function listTaskcoreSkillEntries(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<TaskcoreSkillEntry[]> {
  const root = await resolveTaskcoreSkillsDir(moduleDir, additionalCandidates);
  if (!root) return [];
  ...
  try {
    ...
    return entries
      ...
      .map((entry) => ({
        key: `taskcore/taskcore/${entry.name}`,
...
function normalizeConfiguredTaskcoreRuntimeSkills(value: unknown): TaskcoreSkillEntry[] {
...
export async function readTaskcoreRuntimeSkillEntries(
  config: Record<string, unknown>,
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<TaskcoreSkillEntry[]> {
  const configuredEntries = normalizeConfiguredTaskcoreRuntimeSkills(config.taskcoreRuntimeSkills);
  if (configuredEntries.length > 0) return configuredEntries;
  return listTaskcoreSkillEntries(moduleDir, additionalCandidates);
}

export async function readTaskcoreSkillMarkdown(
  moduleDir: string,
  skillKey: string,
): Promise<string | null> {
...
export function readTaskcoreSkillSyncPreference(config: Record<string, unknown>): {
  explicit: boolean;
  desiredSkills: string[];
} {
  const raw = config.taskcoreSkillSync;
...
function canonicalizeDesiredTaskcoreSkillReference(
  reference: string,
...
export function resolveTaskcoreDesiredSkillNames(
  config: Record<string, unknown>,
  ...
  const desiredSkills = preference.desiredSkills
    .map((reference) => canonicalizeDesiredTaskcoreSkillReference(reference, availableEntries))
    .filter(Boolean);
...
export function writeTaskcoreSkillSyncPreference(
  ...
  next.taskcoreSkillSync = current;
  return next;
}

export async function ensureTaskcoreSkillSymlink(
  source: string,
  target: string,
...
export async function runChildProcess(
  ...
  return new Promise<RunProcessResult>((resolve, reject) => {
    ...
    // These vars leak in when the Taskcore server itself is started from
    // within a Claude Code session (e.g. `npx taskcore run` in a terminal
