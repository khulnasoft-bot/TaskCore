...
Official TypeScript SDK for Taskcore plugin authors.

- **Worker SDK:** `@taskcore/plugin-sdk` — `definePlugin`, context, lifecycle
- **UI SDK:** `@taskcore/plugin-sdk/ui` — React hooks and slot props
- **Testing:** `@taskcore/plugin-sdk/testing` — in-memory host harness
- **Bundlers:** `@taskcore/plugin-sdk/bundlers` — esbuild/rollup presets
- **Dev server:** `@taskcore/plugin-sdk/dev-server` — static UI server + SSE reload
...
| Import | Purpose |
|--------|--------|
| `@taskcore/plugin-sdk` | Worker entry: `definePlugin`, `runWorker`, context types, protocol helpers |
| `@taskcore/plugin-sdk/ui` | UI entry: `usePluginData`, `usePluginAction`, `usePluginStream`, `useHostContext`, slot prop types |
| `@taskcore/plugin-sdk/ui/hooks` | Hooks only |
| `@taskcore/plugin-sdk/ui/types` | UI types and slot prop interfaces |
| `@taskcore/plugin-sdk/testing` | `createTestHarness` for unit/integration tests |
| `@taskcore/plugin-sdk/bundlers` | `createPluginBundlerPresets` for worker/manifest/ui builds |
| `@taskcore/plugin-sdk/dev-server` | `startPluginDevServer`, `getUiBuildSnapshot` |
| `@taskcore/plugin-sdk/protocol` | JSON-RPC protocol types and helpers (advanced) |
| `@taskcore/plugin-sdk/types` | Worker context and API types (advanced) |
...
- Plugin UI bundles run as same-origin JavaScript inside the main Taskcore app. They can call ordinary Taskcore HTTP APIs with the board session, so manifest capabilities are not a frontend sandbox.
...
- For deployed plugins, publish an npm package and install that package into the Taskcore instance at runtime.
...
```ts
import { definePlugin, runWorker } from "@taskcore/plugin-sdk";
...
**Entity types** (for `entityTypes` on slots): `project` \| `issue` \| `agent` \| `goal` \| `run` \| `comment`. Full list: import `PLUGIN_UI_SLOT_TYPES` and `PLUGIN_UI_SLOT_ENTITY_TYPES` from `@taskcore/plugin-sdk`.
...
A card or section rendered on the main dashboard. Use this for at-a-glance metrics, status indicators, or summary views that surface plugin data alongside core Taskcore information. Receives `PluginWidgetProps` with `context.companyId` set to the active company. Requires the `ui.dashboardWidget.register` capability.
...
Full list in code: import `PLUGIN_CAPABILITIES` from `@taskcore/plugin-sdk`.
...
const child = await ctx.issues.create({
  ...
  originKind: "plugin:taskcore.missions:feature",
...
```tsx
import { usePluginData, usePluginAction } from "@taskcore/plugin-sdk/ui";
...
```tsx
import { usePluginData } from "@taskcore/plugin-sdk/ui";
...
import { usePluginAction, type PluginBridgeError } from "@taskcore/plugin-sdk/ui";
...
```tsx
import { useHostContext, usePluginData } from "@taskcore/plugin-sdk/ui";
import type { PluginDetailTabProps } from "@taskcore/plugin-sdk/ui";
...
```tsx
import { usePluginStream } from "@taskcore/plugin-sdk/ui";
...
Each slot type receives a typed props object with `context: PluginHostContext`. Import from `@taskcore/plugin-sdk/ui`.
...
```tsx
import type { PluginProjectSidebarItemProps } from "@taskcore/plugin-sdk/ui";
...
```tsx
import { useState } from "react";
import { usePluginAction, usePluginStream } from "@taskcore/plugin-sdk/ui";
...
```ts
import { createTestHarness } from "@taskcore/plugin-sdk/testing";
...
```ts
import { createPluginBundlerPresets } from "@taskcore/plugin-sdk/bundlers";
...
```bash
taskcore-plugin-dev-server --root . --ui-dir dist/ui --port 4177
```

Or programmatically:

```ts
import { startPluginDevServer } from "@taskcore/plugin-sdk/dev-server";
const server = await startPluginDevServer({ rootDir: process.cwd() });
```

Dev server endpoints:
- `GET /__taskcore__/health` returns `{ ok, rootDir, uiDir }`
- `GET /__taskcore__/events` streams `reload` SSE events on UI build changes
