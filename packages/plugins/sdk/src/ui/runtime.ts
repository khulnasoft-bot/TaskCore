...
type GlobalBridge = typeof globalThis & {
  __taskcorePluginBridge__?: PluginBridgeRegistry;
};

function getBridgeRegistry(): PluginBridgeRegistry | undefined {
  return (globalThis as GlobalBridge).__taskcorePluginBridge__;
}
...
export function renderSdkUiComponent<TProps>(
  ...
  throw new Error(`Taskcore plugin UI component "${name}" is not callable`);
}
