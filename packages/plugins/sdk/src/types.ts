/**
 * Core types for the Taskcore plugin worker-side SDK.
 *
 * These types define the stable public API surface that plugin workers import
 * from `@taskcore/plugin-sdk`.  The host provides a concrete implementation
...
// ---------------------------------------------------------------------------
// Re-exports from @taskcore/shared (plugin authors import from one place)
// ---------------------------------------------------------------------------
...
export interface ScopeKey {
  /** What kind of Taskcore object this state is scoped to. */
...
export interface PluginEntityUpsert {
  ...
  externalId?: string;
  /** Human-readable title for display in the Taskcore UI. */
  title?: string;
...
/**
 * `ctx.events` — subscribe to and emit Taskcore domain events.
 *
...
export interface PluginEventsClient {
  /**
   * Subscribe to a core Taskcore domain event or a plugin-namespaced event.
   *
 ...
 * This client resolves the reference through the Taskcore secret provider
...
export interface PluginSecretsClient {
   ...
   * in the Taskcore secret provider (e.g. `"MY_API_KEY"`).
   *
 ...
 * ```
 *
 * @see usePluginStream in `@taskcore/plugin-sdk/ui`
 */
 ...
 * @example
 * ```ts
 * import { definePlugin } from "@taskcore/plugin-sdk";
 *
