...
function packLocalPackage(packagePath: string, outputDir: string): string {
  ...
  const sdkBundleDir = path.join(outputDir, ".taskcore-sdk");
...
/**
 * Generate a complete Taskcore plugin starter project.
 *
...
export function scaffoldPluginProject(options: ScaffoldPluginOptions): string {
  ...
  const description = options.description ?? "A Taskcore plugin";
  ...
  const packageJson = {
    ...
    scripts: {
      ...
      "dev:ui": "taskcore-plugin-dev-server --root . --ui-dir dist/ui --port 4177",
    ...
    keywords: ["taskcore", "plugin", category],
    author,
    license: "MIT",
    ...(packedSharedTarball
      ? {
        pnpm: {
          overrides: {
            "@taskcore/shared": `file:${toPosixPath(path.relative(outputDir, packedSharedTarball))}`,
          },
        },
      }
      : {}),
    devDependencies: {
      ...(packedSharedTarball
        ...
        : {}),
      "@taskcore/plugin-sdk": sdkDependency,
...
`,
  ...
  writeFile(
    ...
    `import { usePluginAction, usePluginData, type PluginWidgetProps } from "@taskcore/plugin-sdk/ui";
...
function runCli() {
  ...
  if (!pluginName) {
    ...
    console.error("Usage: create-taskcore-plugin <name> [--template default|connector|workspace] [--output <dir>] [--sdk-path <taskcore-sdk-path>]");
    process.exit(1);
  }
