...
async function ensureEmbeddedPostgresConnection(
  ...
  const preferredAdminConnectionString = `postgres://taskcore:taskcore@127.0.0.1:${preferredPort}/postgres`;
  ...
  if (!runningPid && existsSync(pgVersionFile)) {
    try {
      ...
      await ensurePostgresDatabase(preferredAdminConnectionString, "taskcore");
      process.emitWarning(
  ...
  const adminConnectionString = `postgres://taskcore:taskcore@127.0.0.1:${selectedPort}/postgres`;
  await ensurePostgresDatabase(adminConnectionString, "taskcore");

  return {
    connectionString: `postgres://taskcore:taskcore@127.0.0.1:${selectedPort}/taskcore`,
