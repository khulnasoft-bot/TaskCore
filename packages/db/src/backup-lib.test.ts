...
async function createTempDatabase(): Promise<string> {
  const db = await startEmbeddedPostgresTestDatabase("taskcore-db-backup-");
...
describe("createBufferedTextFileWriter", () => {
  it("preserves line boundaries across buffered flushes", async () => {
    const tempDir = createTempDir("taskcore-buffered-writer-");
    ...
    async () => {
      ...
      try {
        await fs.promises.writeFile(
          backupFile,
          [
            "-- Taskcore database backup",
            "-- Created: 2026-04-06T00:00:00.000Z",
            "",
            "BEGIN;",
            "-- taskcore statement breakpoint 69f6f3f1-42fd-46a6-bf17-d1d85f8f3900",
