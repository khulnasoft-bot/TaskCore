...
function resolveTaskcoreInstanceId(): string {
  const raw = process.env.TASKCORE_INSTANCE_ID?.trim() || "default";
...
function resolveDefaultConfigPath(): string {
  return path.resolve(resolveTaskcoreHomeDir(), "instances", resolveTaskcoreInstanceId(), "config.json");
}
...
function resolveDefaultBackupDir(): string {
  return path.resolve(resolveTaskcoreHomeDir(), "instances", resolveTaskcoreInstanceId(), "data", "backups");
}
