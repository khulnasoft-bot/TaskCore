...
async function isLikelyTaskcoreRuntimeSkillPath(
  candidate: string,
  skillName: string,
  ...
  for (let depth = 0; depth < 6; depth += 1) {
    if (await isLikelyTaskcoreRepoRoot(cursor)) return true;
...
async function pruneBrokenUnavailableTaskcoreSkillSymlinks(
  skillsHome: string,
  ...
  for (const entry of entries) {
    ...
    if (
      !(await isLikelyTaskcoreRuntimeSkillPath(resolvedLinkedPath, entry.name, {
    ...
    await onLog(
      "stdout",
      `[taskcore] Removed stale Codex skill "${entry.name}" from ${skillsHome}\n`,
    );
  }
}
...
export async function ensureCodexSkillsInjected(
  ...
  const allSkillsEntries = options.skillsEntries ?? await readTaskcoreRuntimeSkillEntries({}, __moduleDir);
    ...
    try {
      ...
      if (existing?.isSymbolicLink()) {
        ...
        if (
          ...
          await onLog(
            "stdout",
            `[taskcore] Repaired Codex skill "${entry.runtimeName}" into ${skillsHome}\n`,
          );
          continue;
        }
      }
      ...
      await onLog(
        "stdout",
        `[taskcore] ${result === "repaired" ? "Repaired" : "Injected"} Codex skill "${entry.runtimeName}" into ${skillsHome}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[taskcore] Failed to inject Codex skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }

  await pruneBrokenUnavailableTaskcoreSkillSymlinks(
    skillsHome,
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  if (workspaceStrategy) {
    env.TASKCORE_WORKSPACE_STRATEGY = workspaceStrategy;
  }
  if (workspaceId) {
  ...
  if (workspaceBranch) {
    env.TASKCORE_WORKSPACE_BRANCH = workspaceBranch;
  }
  if (workspaceWorktreePath) {
    env.TASKCORE_WORKSPACE_WORKTREE_PATH = workspaceWorktreePath;
  }
  if (agentHome) {
  ...
  if (runtimePrimaryUrl) {
    env.TASKCORE_RUNTIME_PRIMARY_URL = runtimePrimaryUrl;
  }
  ...
  if (runtimeSessionId && !canResumeSession) {
    await onLog(
      "stdout",
      `[taskcore] Codex session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${cwd}".\n`,
    );
  }
  ...
  if (
    ...
    await onLog(
      "stdout",
      `[taskcore] Codex resume session "${sessionId}" is unavailable; retrying with a fresh session.\n`,
    );
