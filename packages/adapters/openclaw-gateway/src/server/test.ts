...
import { asString, parseObject } from "@taskcore/adapter-utils/server-utils";
    ...
    try {
      ...
      if (probeResult === "ok") {
        ...
        checks.push({
          ...
          hint: "Verify network reachability and gateway URL from the Taskcore server host.",
        });
      }
    } catch (err) {
      checks.push({
