...
Use when:
- You want Taskcore to invoke OpenClaw over the Gateway WebSocket protocol.
...
- Your deployment does not permit outbound WebSocket access from the Taskcore server.
...
- taskcoreApiUrl (string, optional): absolute Taskcore base URL advertised in wake text
- claimedApiKeyPath (string, optional): path to the claimed API key JSON file read by the agent at wake time (default ~/.openclaw/workspace/taskcore-claimed-api-key.json)
