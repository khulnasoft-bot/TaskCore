...
## Scope
This plan is now **gateway-only**. Taskcore supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses Taskcore skill.
...
3. In `/new` OpenClaw session, OpenClaw can still create a Taskcore task.
...
```bash
pnpm --filter @taskcore/server test:run -- openclaw-gateway
pnpm --filter @taskcore/server typecheck
pnpm --filter @taskcore/ui typecheck
pnpm --filter taskcore typecheck
```
