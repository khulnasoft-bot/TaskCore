...
import { removeMaintainerOnlySkillSymlinks } from "@taskcore/adapter-utils/server-utils";
...
async function ensureOpenCodeSkillsInjected(
  ...
  for (const skillName of removedSkills) {
    await onLog(
      "stderr",
      `[taskcore] Removed maintainer-only OpenCode skill "${skillName}" from ${skillsHome}\n`,
    );
  }
  for (const entry of selectedEntries) {
    ...
    try {
      ...
      await onLog(
        "stderr",
        `[taskcore] ${result === "repaired" ? "Repaired" : "Injected"} OpenCode skill "${entry.key}" into ${skillsHome}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[taskcore] Failed to inject OpenCode skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const openCodeSkillEntries = await readTaskcoreRuntimeSkillEntries(config, __moduleDir);
  const desiredOpenCodeSkillNames = resolveTaskcoreDesiredSkillNames(config, openCodeSkillEntries);
  ...
  try {
    ...
    if (runtimeSessionId && !canResumeSession) {
      await onLog(
        "stdout",
        `[taskcore] OpenCode session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${cwd}".\n`,
      );
    }
    ...
    if (
      ...
      await onLog(
        "stdout",
        `[taskcore] OpenCode session "${sessionId}" is unavailable; retrying with a fresh session.\n`,
      );
