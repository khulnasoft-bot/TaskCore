...
Use when:
- You want Taskcore to run Cursor Agent CLI locally as the agent runtime
...
- Taskcore auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$taskcore" and related skills on local runs.
- Taskcore auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
`;
