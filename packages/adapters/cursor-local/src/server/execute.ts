...
export async function ensureCursorSkillsInjected(
  ...
  const skillsEntries = options.skillsEntries
    ?? (options.skillsDir
      ...
      : await readTaskcoreRuntimeSkillEntries({}, __moduleDir));
  ...
  try {
    ...
    await onLog(
      "stderr",
      `[taskcore] Failed to prepare Cursor skills directory ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
    );
    return;
  }
  ...
  for (const skillName of removedSkills) {
    await onLog(
      "stderr",
      `[taskcore] Removed maintainer-only Cursor skill "${skillName}" from ${skillsHome}\n`,
    );
  }
  ...
  for (const entry of skillsEntries) {
    ...
    try {
      ...
      await onLog(
        "stderr",
        `[taskcore] ${result === "repaired" ? "Repaired" : "Injected"} Cursor skill "${entry.key}" into ${skillsHome}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[taskcore] Failed to inject Cursor skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const cursorSkillEntries = await readTaskcoreRuntimeSkillEntries(config, __moduleDir);
  const desiredCursorSkillNames = resolveTaskcoreDesiredSkillNames(config, cursorSkillEntries);
  ...
  if (wakeTaskId) {
    env.TASKCORE_TASK_ID = wakeTaskId;
  }
  if (wakeReason) {
    env.TASKCORE_WAKE_REASON = wakeReason;
  }
  if (wakeCommentId) {
    env.TASKCORE_WAKE_COMMENT_ID = wakeCommentId;
  }
  if (approvalId) {
    env.TASKCORE_APPROVAL_ID = approvalId;
  }
  if (approvalStatus) {
    env.TASKCORE_APPROVAL_STATUS = approvalStatus;
  }
  if (linkedIssueIds.length > 0) {
    env.TASKCORE_LINKED_ISSUE_IDS = linkedIssueIds.join(",");
  }
  if (wakePayloadJson) {
    env.TASKCORE_WAKE_PAYLOAD_JSON = wakePayloadJson;
  }
  if (effectiveWorkspaceCwd) {
    env.TASKCORE_WORKSPACE_CWD = effectiveWorkspaceCwd;
  }
  if (workspaceSource) {
    env.TASKCORE_WORKSPACE_SOURCE = workspaceSource;
  }
  if (workspaceId) {
    env.TASKCORE_WORKSPACE_ID = workspaceId;
  }
  if (workspaceRepoUrl) {
    env.TASKCORE_WORKSPACE_REPO_URL = workspaceRepoUrl;
  }
  if (workspaceRepoRef) {
    env.TASKCORE_WORKSPACE_REPO_REF = workspaceRepoRef;
  }
  if (agentHome) {
    env.AGENT_HOME = agentHome;
  }
  if (workspaceHints.length > 0) {
    env.TASKCORE_WORKSPACES_JSON = JSON.stringify(workspaceHints);
  }
  ...
  if (runtimeSessionId && !canResumeSession) {
    await onLog(
      "stdout",
      `[taskcore] Cursor session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${cwd}".\n`,
    );
  }
  ...
  const promptMetrics = {
    ...
    runtimeNoteChars: taskcoreEnvNote.length,
  ...
  if (
    ...
    await onLog(
      "stdout",
      `[taskcore] Cursor resume session "${sessionId}" is unavailable; retrying with a fresh session.\n`,
    );
