...
Use when:
- You want Taskcore to run Pi (the AI coding agent) locally as the agent runtime
...
- Taskcore requires an explicit \`model\` value for \`pi_local\` agents.
- Sessions are stored in ~/.pi/taskcores/ and resumed with --session.
